...
from lib.interpolator import Interpolator3D
...
TENOR_BUMP = 1e-2
@dataclass
class VolSurfaceLV(VolSurfaceInterp):
    _rate: float = 0
    ...
    def get_tenor_vol(self, dcf: float, strike: float, forward_price: float) -> float:
        ...
        vol = np.sqrt((opt_dt + self.rate * price_k) / (0.5 * strike**2 * opt_dkk))
        return vol

@dataclass
class VolSurfaceSABR(VolSurfaceBase):
    volvol: float
    beta: float
    rho: float
    alpha: float
