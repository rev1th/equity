...
from models.vol_surface import VolSurfaceBase, VolSurfaceInterp, VolSurfaceLV, VolSurfaceSABR
from lib import option_analytics
...
class VolSurfaceModel(NameDateClass):
    ...
    def build_implied(self):
        ...
        for (expiry, strike), (call_opt, put_opt) in self._option_chain.items():
            ...
            if call_opt.is_valid():
                call_vol = call_opt.get_implied_vol(rate=self.rate)
                res.append([expiry, strike, call_vol])
            if put_opt.is_valid():
                put_vol = put_opt.get_implied_vol(rate=self.rate)
    ...
    def build_LV(self):
        ...
        return VolSurfaceLV(self.date, vs_implied._nodes, _rate=self.rate)
    ...
    def build_SABR(self, beta: float = 1):
        ...
        alpha = option_analytics.get_SABR_alpha(vol_atm=vol_atm, forward_price=underlier.price,
        ...
        return VolSurfaceSABR(self.date, volvol=volvol, beta=beta, rho=rho, alpha=alpha)
    ...
    def get_calibration_errors(self, vol_surface: VolSurfaceBase):
        ...
        for (expiry, strike), (call_opt, put_opt) in self._option_chain.items():
            if call_opt.is_valid():
                call_error = call_opt.get_price(vol_surface, rate=self.rate) - call_opt.price
                res.append([expiry, strike, call_error])
            if put_opt.is_valid():
                put_error = put_opt.get_price(vol_surface, rate=self.rate) - put_opt.price
